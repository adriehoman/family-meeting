...
| `src/cherry-docs.ps1` | PowerShell script (needs Microsoft Word; run it directly with `& .\cherry-docs.ps1`) that produces the Cherry Wilkes teacher guide, observer quiz (with Word tick boxes), the quiz Benchmark Answers (BA) and the student hints. The checklist is read from `scenarios.js`. |
